# AuraPDF — Complete Developer Documentation

> **Version:** 2.0  
> **Last Updated:** August 2026  
> **Purpose:** This document is the single source of truth for reimplementing every feature of the AuraPDF application. It contains full architecture diagrams, data flows, algorithm explanations, API references, database schemas, and frontend component specifications.
...
5. [Backend Modules](#5-backend-modules)
   - 5.1 [main.py — FastAPI Application](#51-mainpy--fastapi-application)
   - 5.2 [pdf_converter.py — Image Processing Engine](#52-pdf_converterpy--image-processing-engine)
   - 5.3 [storage.py — Abstract Storage Layer](#53-storagepy--abstract-storage-layer)
   - 5.4 [task_queue.py — Background Job Queue](#54-task_queuepy--background-job-queue)
   - 5.5 [database.py — SQLite + Auth + Encrypted Credentials](#55-databasepy--sqlite--auth--encrypted-credentials)
   - 5.6 [llm_adapter.py — Multi-Provider AI Adapters](#56-llm_adapterpy--multi-provider-ai-adapters)
   - 5.7 [rag_indexer.py — RAG Vector Search](#57-rag_indexerpy--rag-vector-search)
6. [Frontend Modules](#6-frontend-modules)
   - 6.1 [Main Converter Page (index.html + app.js)](#61-main-converter-page)
   - 6.2 [Enhanced AI Reader (reader_enhanced.html)](#62-enhanced-ai-reader)
   - 6.3 [Reader Core JS Modules](#63-reader-core-js-modules)
7. [Feature Reference](#7-feature-reference)
   - 7.1 [PDF Dark-Mode Conversion](#71-pdf-dark-mode-conversion)
   - 7.2 [Theme Presets and Custom Colors](#72-theme-presets-and-custom-colors)
   - 7.3 [Smart Invert](#73-smart-invert)
   - 7.4 [Text Brightness Boost](#74-text-brightness-boost)
   - 7.5 [Resolution DPI](#75-resolution-dpi)
   - 7.6 [Image Quality](#76-image-quality)
   - 7.7 [Parallel Workers Threads](#77-parallel-workers-threads)
   - 7.8 [Live Preview](#78-live-preview)
   - 7.9 [Preview Page Number](#79-preview-page-number)
   - 7.10 [User Authentication and History](#710-user-authentication-and-history)
   - 7.11 [Multi-Provider AI Chat](#711-multi-provider-ai-chat)
   - 7.12 [RAG Retrieval-Augmented Generation](#712-rag-retrieval-augmented-generation)
   - 7.13 [EPUB and Markdown Support](#713-epub-and-markdown-support)
   - 7.14 [Text-to-Speech TTS](#714-text-to-speech-tts)
   - 7.15 [Performance Monitor Telemetry](#715-performance-monitor-telemetry)
...
The `terraform/` directory contains IaC for deploying to Google Cloud Run with:
- Cloud Storage for files
- BigQuery for metadata pipeline

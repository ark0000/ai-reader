...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (panel) {
    ...
    // Initialize AI Chat Brightness
    const brightnessSlider = document.getElementById('ai-brightness-slider');
    if (brightnessSlider) {
      const savedBrightness = window.safeStorage ? window.safeStorage.getItem('ai-chat-brightness') : localStorage.getItem('ai-chat-brightness');
      if (savedBrightness) {
        brightnessSlider.value = savedBrightness;
        panel.style.setProperty('--ai-brightness', savedBrightness);
      }
      brightnessSlider.addEventListener('input', (e) => {
        const val = e.target.value;
        panel.style.setProperty('--ai-brightness', val);
        if (window.safeStorage) {
          window.safeStorage.setItem('ai-chat-brightness', val);
        } else {
          localStorage.setItem('ai-chat-brightness', val);
        }
      });
    }
  }
  ...
  if (window.switchTab && window.togglePanel) {
    ...
    if (input) {
      if (data.type === 'code') {
        ...
        input.value = "Explain this diagram/image.\n";
      }
      input.focus();
    }
  }
